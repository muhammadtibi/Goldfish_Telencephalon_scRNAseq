...
%%
direct='/data/Technion_analysis/goldfish/scRNAseq_gf/comparative/nn_mouse_gaba';
cd(direct);
load('/bigdata/all_mouse_dataset/mn_markeragg.mat','mn_markeragg','mn_clagg')
load([direct,'/m_vs_gf_h.mat'],"prj_all","perplexity","num_har","all_flags","data_orig_all","geneid","geneid_all","data",'mapped_xy','mapped_xy_start');% prj is harmony 0
...
%% calc. nearst nbr method
    ...
    load('cluter_param',"idx","MinPts","eps_prc")
    clidx=idx;
    %% or dbsacn 
    eps_prc=50;
    MinPts=45;
    dbscanit
    clidx=idx;
    save('cluter_param',"idx","MinPts","eps_prc")
    %% or KNNmat instead (tranfer label)
        ...
        mm=sum(mgf==1);
        m_xy=mapped_xy(1:mm,:);
        g_xy=mapped_xy((mm+1):end,:);
        disty=pdist(mapped_xy);
        dd=squareform(disty);
    ...
    Tlv2ct=table([["cell type","location",n_g'];[n_m,locia(1:length(n_m))',lv2ct]]);
    ...
    marker=geneid_all(ind_gr_tmp_mark);
    for i=1:length(marker)
        in=find(strcmpi(marker(i),geneid_all));
        inall=[inall;in];
    end
    ...
    colormap('summer');
    %% firstly-coscatter with enrichment to all genes-all cluster
    T_cells_tmp=igf;
    [rs,is]=sort(T_cells_tmp,'ascend');
    T_cells_tmp=T_cells_tmp(is);
    all_data=data_orig_all(:,is);
    [ind_gr_tmp_mark,vi0] = markertablefeatures_tmp(T_cells_tmp,all_data,length(geneid_all),1);% top_genes by me
    %% then- coscatter: extract two wanted clusters  :
    ct1="TEGLU13";
    ct2="g_GLUT_2_CBLN1_EMX1";
    m1=find(ugf==ct1);
    g1=find(ugf==ct2);
    [~,mi]=sort(ind_gr_tmp_mark(:,m1));
    [~,gi]=sort(ind_gr_tmp_mark(:,g1));
    md=vi0(mi,m1);
    gd=vi0(gi,g1);
    figure('color','w');
    s=scatter(md,gd,'.');
    row = dataTipTextRow("gene",geneid_all);
    s.DataTipTemplate.DataTipRows(end+1) = row;
    axis equal; axis tight;
    xlabel('m')
    ylabel('gf')
    title([ct1,' VS ',ct2],'Interpreter','none');
    %% GCA: gene coreelation analysis
    % create mean datsets
    mean_data1=zeros(length(geneid),length(ugf));

    for cc=1:length(ugf)
        cc
        gxy = data(:,strcmpi(ugf(cc),nms));
        mean_data1(:,cc)=mean(gxy,2);
    end
    mean_m=mean_data1(:,1:length(n_m));
    mean_gf=mean_data1(:,(length(n_m)+1):end);

    % calc. RHO bewtwenn gf & m
    [RHO,~] = corr(mean_m,mean_gf,'Type','Spearman');
    % tops
    ...
    r_top=topdv(1:10);
    % figure 
    Zpcay = linkage(RHO,'ward','correlation');
    Dx = pdist(RHO,'correlation');
    leafordery = optimalleaforder(Zpcay,Dx);
    datamarkers= RHO(leafordery,:);
    figure('color','w')
    imagesc(cent_norm(log2(datamarkers+1)));
    colormap(gray)
    set(gca,'xtick',1:length(n_g),'xticklabel',n_g, 'fontsize', 10)
    set(gca,'ytick',1:length(n_m),'yticklabel',n_m(leafordery), 'fontsize', 10)
    %% heatmap of expression sorted by RHO
    figure;
    set(gcf,'color','w');
    inall=[];
    %marker = gname;%,'GSTP1','PTGDS','AQP1'};%{'STMN2','SNAP25','CLDN5','FLT1','EPAS1','ID3','GFAP','HES5','SLC1A3','ID4','SLC4A4','MFGE8','SLC1A2','SLC6A11','MPZ','PLP1','cldn19','NINJ2','SLC6A9','SOX9','OLIG2','OLIG1','SOX10','TRAF4','TAGLN','S100B'};
    ...
    imagesc(lv2ct(is,:));
    colormap()
    set(gca,'ytick',1:length(n_m),'yticklabel',n_m(is), 'fontsize', 10)
    set(gca,'xtick',1:length(n_g),'xticklabel',n_g, 'fontsize', 10)
    colorbar 

    %% heatmap- subset !dbscan 
    d_mat=pdist(lv2ct');
...
for x=1:length(n_gx)
    ...
    text(textPosition(x,1), textPosition(x,2), ...
        num2str(x), 'HorizontalAlignment','center','FontSize',5,'FontWeight','bold');%, ...        % 'BackgroundColor',[0.8 0.8 0.8]
...
scatter(mapped_xy(mgf==2,1),mapped_xy(mgf==2,2),20,markergene_color2((mgf==2),:),'.'); % mouse
axis off; axis tight; axis equal;
hold on
scatter(mapped_xy(mgf==1,1),mapped_xy(mgf==1,2),20,markergene_color((mgf==1),:),'.'); % mouse
axis off; axis tight; axis equal;
title(fg)
