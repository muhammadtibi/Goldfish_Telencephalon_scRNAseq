...
if 1 % to run all sections
    ...
    disp('extra m&b')
    direct='/data/Technion_analysis/goldfish/scRNAseq_gf/comparative/nn_gf_mouse';% get save folder
    load('/data/Technion_analysis/goldfish/scRNAseq_gf/comparative/nn_gf_mouse/nn_mg_10x.mat')
...
%         fetsel="TF";
       ...
       elseif fetsel=="mt_tmp"
        ...
        elseif fetsel=="TF"
   TF=readcell('/data/Technion_analysis/goldfish/scRNAseq_gf/comparative/TFs-MouseCellAtlas.txt');
   TF=upper(string(TF(:,1)));  
   corr_filt=zeros(length(TF),1);
      tf=1:length(geneid);
      for t=1:length(TF)
          try
      corr_filt(t)=tf(contains(geneid,TF(t)));
          end
      end
       corr_filt(corr_filt==0) = [];
        data = data(corr_filt,:);
           geneid = geneid(corr_filt);
                else % "Spatial"
            pdist_vis
...
%             end
  ...
  if savefig_flag==1
    ...
    prj=prj_all{end};% {itr+1} % CHANGE HERE!!
    cd(direct)
    disp('projecting...')
...
%     end
    ...
    disp('plot flags')
    if flags==1
        for ii=1:size(all_flags,2) % 2 is all cell types not needed
...
elseif ii==3
legend({'GABA','DOPA','GLUT'})
elseif ii==4
legend({'m-GABA','m-DOPA','m-GLUT','g-GABA','geneidg-GLUT'})
else
...
savefig(gcf,['tsne_all_flags_',num2str(ii),'.fig'])
    ...
    %% save 
    disp('save')
    save([direct,'/m_vs_gf_h.mat'],"prj_all","perplexity","geneid",'mapped_xy','all_flags','num_har','geneid_all','data_orig_all','data','-v7.3')
    set(0,'DefaultFigureWindowStyle','normal')
