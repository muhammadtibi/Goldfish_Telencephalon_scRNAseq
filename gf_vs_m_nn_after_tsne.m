...
clc
%%
direct='/data/Technion_analysis/goldfish/scRNAseq_gf/comparative/nn_gf_mouse';
cd(direct);
load('/bigdata/all_mouse_dataset/mnn_markeragg.mat','mnn_markeragg','mnn_clagg')
load([direct,'/m_vs_gf_h.mat'],"prj_all","perplexity","num_har","all_flags","data_orig_all","geneid_all","data",'mapped_xy','mapped_xy_start');% prj is harmony 0
...
nonn=summer(length(n_g));
%% optinal find previous flag from loom 
load('/bigdata/all_mouse_dataset/loom_nn.mat','mnn_cln','mnn_t3')
%% 
[ucln,icln]=unique(mnn_cln,'stable');
umnn=mnn_t3(icln);
tx3=strings(size(nms));
for i=1:length(ucln)
    xi=find(nms==ucln(i));
    tx3(xi)=umnn(i);
end
...
%     ystr=strcat(rgni(is),'-',n_m(is));
%     set(gca,'ytick',1:length(n_m),'yticklabel',strrep(ystr,' ',''), 'fontsize', 10)
...
colormap('summer');
%% coscatter with enrichment to all genes-all cluster 
 T_cells_tmp=igf;
[rs,is]=sort(T_cells_tmp,'ascend');
T_cells_tmp=T_cells_tmp(is);
all_data=data_orig_all(:,is);
[ind_gr_tmp_mark,vi0] = markertablefeatures_tmp(T_cells_tmp,all_data,length(geneid_all),1);% top_genes by me
% coscatter: extract two wanted clusters  :
% md=all_data(:,flag_mgf==1),2);
% gd=all_data(:,flag_mgf==2),2);
figure;
scatter(md,gd)
    %% create dendrogram
    
%     appended=append('nn',string(1:length(n_g)));
    appended=append('g-',n_g);
    nugf=[n_m;appended];
    % y= clusters
...
%     axes('position',[0.03,0.1,0.3,0.8])
    ...
    writetable(Tlv2ct,[direct,'/','nn_Tlv2ct.csv']);
%% hm 
figure('color','w')
subplot(1,2,1)
...
for i=1:length(n_gx)
    ...
    scatter(mapped_xy(xgf,1),mapped_xy( xgf,2),30,nonn(i,:),'.'); % mouse
...
for x=1:length(n_gx)
    ...
    text(textPosition(x,1), textPosition(x,2), ...
        ng_y(x), 'HorizontalAlignment','center','FontSize',5,'FontWeight','bold');%, ...        % 'BackgroundColor',[0.8 0.8 0.8]
%num2str(x) or 
end
axis off;  axis equal;
...
%% % % mouse (supra-regions)
ut3=unique(tx3);
clr=distinguishable_colors(length(ut3));
